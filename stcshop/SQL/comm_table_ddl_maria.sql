...
CREATE TABLE STCTCCMMNCODE (
  ...
  PRIMARY KEY (CODE_ID),
  KEY R_179 (CL_CODE),
  CONSTRAINT STCTCCMMNCODE_ibfk_1 FOREIGN KEY (CL_CODE) REFERENCES STCTCCMMNCLCODE (CL_CODE)
) ;
...
CREATE TABLE STCTCCMMNDETAILCODE (
  ...
  CONSTRAINT STCTCCMMNDETAILCODE_ibfk_1 FOREIGN KEY (CODE_ID) REFERENCES STCTCCMMNCODE (CODE_ID)
) ;



CREATE TABLE STCTNORGNZTINFO (
...
CREATE TABLE STCTNEMPLYRINFO (
  ...
  KEY STCTNEMPLYRINFO_ibfk_2 (GROUP_ID),
  KEY STCTNEMPLYRINFO_ibfk_1 (ORGNZT_ID),
  CONSTRAINT STCTNEMPLYRINFO_ibfk_2 FOREIGN KEY (GROUP_ID) REFERENCES STCTNAUTHORGROUPINFO (GROUP_ID) ON DELETE CASCADE,
  CONSTRAINT STCTNEMPLYRINFO_ibfk_1 FOREIGN KEY (ORGNZT_ID) REFERENCES STCTNORGNZTINFO (ORGNZT_ID) ON DELETE CASCADE
) ;



CREATE TABLE STCTNTMPLATINFO (
...
CREATE TABLE STCTNBBSUSE (
  ...
  CONSTRAINT STCTNBBSUSE_ibfk_1 FOREIGN KEY (BBS_ID) REFERENCES STCTNBBSMASTER (BBS_ID)
) ;



CREATE TABLE STCTNBBS (
  ...
  KEY STCTNBBS_ibfk_1 (BBS_ID),
  CONSTRAINT STCTNBBS_ibfk_1 FOREIGN KEY (BBS_ID) REFERENCES STCTNBBSMASTER (BBS_ID)
) ;



CREATE TABLE STCTNBBSMASTEROPTN (
...
CREATE TABLE STCTNENTRPRSMBER (
  ...
  KEY STCTNENTRPRSMBER_ibfk_1 (GROUP_ID),
  CONSTRAINT STCTNENTRPRSMBER_ibfk_1 FOREIGN KEY (GROUP_ID) REFERENCES STCTNAUTHORGROUPINFO (GROUP_ID) ON DELETE CASCADE
) ;



CREATE TABLE STCTNFILE (
...
CREATE TABLE STCTNFILEDETAIL (
  ...
  CONSTRAINT STCTNFILEDETAIL_ibfk_1 FOREIGN KEY (ATCH_FILE_ID) REFERENCES STCTNFILE (ATCH_FILE_ID)
) ;



CREATE TABLE STCTNGNRLMBER (
  ...
  ESNTL_ID char(20) NOT NULL,
  PRIMARY KEY (MBER_ID),
  KEY STCTNGNRLMBER_ibfk_1 (GROUP_ID),
  CONSTRAINT STCTNGNRLMBER_ibfk_1 FOREIGN KEY (GROUP_ID) REFERENCES STCTNAUTHORGROUPINFO (GROUP_ID) ON DELETE CASCADE
) ;



CREATE TABLE STCTNSCHDULINFO (
...
CREATE TABLE TN_USR_INFO (
  ...
  SNS_TYPE VARCHAR(10) COMMENT '소셜로그인타입',
  ...
  GROUP_ID CHAR(20) NOT NULL COMMENT '권한그룹ID',
...
CREATE TABLE TH_TRND_INQ_HST (
  ...
  USR_ID VARCHAR(16) NOT NULL COMMENT '사용자ID',
  SRCH_KWRD VARCHAR(20) COMMENT '검색키워드',
  GDS_CLSF1 VARCHAR(20) COMMENT '상품첫번째분류',
  GDS_CLSF2 VARCHAR(20) COMMENT '상품두번째분류',
  GDS_CLSF3 VARCHAR(20) COMMENT '상품세번째분류',
  GDS_CLSF4 VARCHAR(20) COMMENT '상품네번째분류',
  SRCH_DT DATE COMMENT '검색일시',
  INQ_PRD_SCTN VARCHAR(5) COMMENT '조회기간구간단위',
  INQ_PRD_BGNG_YMD VARCHAR(8) COMMENT '조회기간시작일',
  INQ_PRD_END_YMD VARCHAR(8) COMMENT '조회기간종료일',
  DEVICE_TYPE VARCHAR(5) COMMENT '기기종류',
